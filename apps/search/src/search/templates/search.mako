  ...
  if (_mFrom.isValid()) {
    ...
    if (_minMaxDiff > 0 && _minMaxDiff <= 86400) { // max 1 day
      if (_hasWidget && moment(widget.properties.max()).date() != moment(widget.properties.min()).date()){
        _format = _format.substring(6, _format.length);
      }
      else {
        _format = _format.substring(10, _format.length);
      }
    }
    if (_minMaxDiff > 86400 && _minMaxDiff <= 2592000) { // max 1 month
      _format = _format.substring(8, _format.length);
    }
    if (_minMaxDiff > 2592000 && _minMaxDiff <= 22464000) { //360 days
      _format = _format.substring(6, _format.length);
    }
